...
UnifiedBaseballEnv + CurriculumManager + HealthMonitor + RewardAuditSystem 
...
from training.multimodal_networks import MultimodalBatterPolicy, MultimodalValueNetwork, get_device
...
from training.trainer import TrainingConfig, PPOUpdater  # GAE 계산에 재사용
...
def run_curriculum_training(
    ...
    # GAE 계산기 초기화 (trainer.py의 PPOUpdater 재사용)
    ppo_config = TrainingConfig()
    ppo_config.gamma = 0.99
    ppo_config.gae_lambda = 0.95
    ppo_config.n_epochs = 4          # 하드코딩된 3 대신 config에서 관리
    ppo_config.batch_size = 64
    ppo_config.clip_eps = 0.2
    ppo_config.entropy_coef = 0.02
    ppo_config.value_coef = 0.5
    ppo_config.max_grad_norm = 0.5
    updater = PPOUpdater(ppo_config, device)
    ...
    for ep in range(1, total_episodes + 1):
        ...
        if len(obs_list) > 1:
            ...
            # ── GAE 계산 (이전 MC Returns 대체) ──
            values_np = np.array(val_list, dtype=np.float32)
            rewards_np = np.array(rew_list, dtype=np.float32)
            dones_np = np.zeros(len(rew_list), dtype=np.float32)
            dones_np[-1] = 1.0  # 마지막 스텝은 done

            # 다음 상태의 가치 추정 (Bootstrap)
            with torch.no_grad():
                last_obs_t = {k: v[-1:] for k, v in batch_obs.items()}
                next_val = value_net(last_obs_t).squeeze().cpu().item()

            advantages_np, returns_np = updater.compute_gae(
                rewards_np, values_np, next_val, dones_np
            )
            ...
            for _ in range(ppo_config.n_epochs):  # config 기반 epoch 수
        ...
        if ep % 20 == 0 or ep == total_episodes:
            ...
            print("  " + "=" * 52)
            print(f"   ⚾ 10-DOF ROBOT | {inning_str} | OUT: {ge.outs} | HOME {ge.score_home} - {ge.score_away} AWAY")
            print(f"   🟢 Ball: {ge.balls}  🔴 Strike: {ge.strikes} | 🏃 주자: {r_str}")
            print("  " + "=" * 52)
...
if __name__ == "__main__":
    run_curriculum_training(total_episodes=1000, initial_stage=1, record_interval=50)
