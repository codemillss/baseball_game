...
def simulate_pitcher_vs_batter_match(
    pitcher_model_path="checkpoints/mlb_pitcher_best/best_model",
    pitcher_norm_path="checkpoints/mlb_pitcher_vec_normalize.pkl",
    batter_model_path="checkpoints/mlb_batter_best/best_model",
    batter_norm_path="checkpoints/mlb_batter_vec_normalize.pkl",
    num_at_bats=15,
    output_filename="mlb_baseball_match.mp4"
):
    print(f"\n=======================================================")
    ...
    def make_b_env(): return BatterMatchTrainingEnv(is_eval_mode=True)
    ...
    for ab in range(num_at_bats):
        ...
        while not done and step < 70:
            ...
            pitch_type_idx = match_env.target_pitch_type
            pitch_names = ["FAST", "SLIDER", "CURVE"]
            pitch_name = pitch_names[pitch_type_idx]
            
            # Draw header banner
            cv2.rectangle(bgr_frame, (0, 0), (640, 45), (20, 20, 20), -1)
            cv2.putText(bgr_frame, f"AT-BAT {ab+1:2d}/{num_at_bats} | {pitch_name} | {p_speed:.1f} km/h", (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 2)
            cv2.putText(bgr_frame, f"H: {stats['HITS']} | HR: {stats['HOMERUNS']} | K: {stats['STRIKEOUTS']}", (410, 30),
        ...
        pitch_type_idx = match_env.target_pitch_type
        pitch_names = ["FASTBALL", "SLIDER", "CURVE"]
        pitch_name = pitch_names[pitch_type_idx]
        
        print(f"  At-Bat {ab+1:2d}: {outcome:26s} | {pitch_name} | Pitch: {info['pitch_speed_kmh']:.1f} km/h | ExitVel: {info['exit_velocity_kmh']:.1f} km/h | Dist: {info['batted_distance']:.1f}m")
